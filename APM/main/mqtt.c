/* mqtt.c 
	...
	All MQTT messaging code, including the task loop
...
//
// ------------------------------------------------------------------------
//static void send_mqtt_alert_msg(ENUM_MQTT_CHANNELS ch_num, char *p2Ch_name, char *p2ClientID, esp_mqtt_client_handle_t clientHandle )
...
//    int 				msg_id;
	...
	if (mIsMqttEnabled == 1)
	{
		...
		(void)esp_mqtt_client_publish(mqtt_alert_client, "mone", charBuffer, 0, 2, 0);	// Length=0 (publish full string), QOS=2
	}
}	
...
static void send_mqtt_msg(ENUM_MQTT_CHANNELS ch_num, char *p2Ch_name, char *p2ClientID, esp_mqtt_client_handle_t clientHandle )
{
	...
	len = strlen( charBuffer );
	sprintf( &charBuffer[len], "|latitude,0.0" );
	len = strlen( charBuffer );
	sprintf( &charBuffer[len], "|longitude,0.0" );
	len = strlen( charBuffer );
	...
	len = strlen( charBuffer );
	sprintf( &charBuffer[len], "|days_to_maintenance,%d", (int)mqtt_databuf[E_DAYS_TO_MAINT]);
	len = strlen( charBuffer );
	...
	len = strlen( charBuffer );
	sprintf( &charBuffer[len], "|temp_1,%.6f", mqtt_databuf[E_UPPER_BOUND]);
	len = strlen( charBuffer );
	sprintf( &charBuffer[len], "|temp_2,%.6f", mqtt_databuf[E_LOWER_BOUND]);
	len = strlen( charBuffer );
	...
	msg_id = esp_mqtt_client_publish(clientHandle, "mone", charBuffer, 0, 2, 0);	// Length=0 (publish full string), QOS=2

	//	(void)esp_mqtt_client_publish(mqtt_client, "mone", charBuffer, 0, 2, 0);	// Length=0 (publish full string), QOS=2
	//	msg_id = esp_mqtt_client_publish(mqtt_client, "mone", charBuffer, 0, 2, 0);	// Length=0 (publish full string), QOS=2
		
	ESP_LOGI(TAG, "sent publish to topic 'mone', msg_id=%d", msg_id);

}	



// Send all channels to server
static void send_mqtt_msgs()
{
	#ifndef OUTPUT_AI_DATA
	ESP_LOGI(TAG, " *** Sending MQTT channels to Server");
	#endif
	
	// send GYRO_NOW
	updateDataBuffer(E_MQTT_ID0, mqtt_databuf);
	send_mqtt_msg(E_MQTT_ID0, "gyro_now", (char *)mqtt_cfg.client_id, mqtt_client);
	
	// send GYRO	
	updateDataBuffer(E_MQTT_ID1, mqtt_databuf);
	send_mqtt_msg(E_MQTT_ID1, "gyro", (char *)mqtt_cfg.client_id, mqtt_client);
	
	// send TEMPERATURE
	updateDataBuffer(E_MQTT_ID2, mqtt_databuf);
	send_mqtt_msg(E_MQTT_ID2, "temperature", (char *)mqtt_cfg.client_id, mqtt_client);
	
	// send HUMIDITY	
	updateDataBuffer(E_MQTT_ID3, mqtt_databuf);
	send_mqtt_msg(E_MQTT_ID3, "humidity", (char *)mqtt_cfg.client_id, mqtt_client);
	
	// send SOUND	
	updateDataBuffer(E_MQTT_ID4, mqtt_databuf);
	send_mqtt_msg(E_MQTT_ID4, "sound", (char *)mqtt_cfg.client_id, mqtt_client);
...
void mqtt_task(void *arg)
{
	...
	//char 				charBuffer[200];		// must be cautious with the size here so we don't overrun the buffer or exceed stack size
	#endif
    ...
    while (1) {
		...
		gIsReadyForMQTT = 0;					// used to synchronize the MQTT transmissions with the completion of ADC sampling
		xSemaphoreGive(mqtt_mux);	
		while (gIsReadyForMQTT == 0) {
			vTaskDelay(10 / portTICK_RATE_MS);	// wait another 10 msec
		}			
...
void initMqtt( int rateMsec )
{
	...
	mqtt_cfg.host = "xx.xx.xx.xx";	
	mqtt_cfg.port = 0;
	mqtt_cfg.client_id = "xx.xx.xx.xx.xx.xx";		
	...
	#ifdef MQTT_ALERT_ENABLE
	mqtt_alert_cfg.host = "xx.xx.xx.xx";	
	mqtt_alert_cfg.port = 0;
	mqtt_alert_cfg.client_id = "xx.xx.xx.xx.xx.xx";
